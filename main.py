...
def parse_duration(text):
    ...
    فۆرماتی پشتگیریکراو:
      ...
      1س          → 1 سەحات
      1ر          → 1 ڕۆژ
      1ر 2س 3خ   → 1 ڕۆژ، 2 سەحات، 3 خولەک
    دەگەڕێنێتەوە: کۆی خولەکەکان (int) یان None ئەگەر هەڵە بوو
    """
    text = text.strip()
    total = 0
    found = False
    for match in re.finditer(r'(\d+)\s*([رسخ])', text):
    ...
    if not found:
        try:
            return int(text)  # تەنیا ژمارە = خولەک
        except:
            return None
...
def process_add_sub_step1(message):
    try:
        ...
        msg = bot.send_message(message.chat.id,
            ...
            "نموونەکان:\n"
            "• <code>1خ</code> ← 1 خولەک\n"
            "• <code>1س</code> ← 1 سەحات\n"
            "• <code>1ر</code> ← 1 ڕۆژ\n"
            "• <code>1ر 2س 3خ</code> ← 1 ڕۆژ، 2 سەحات، 3 خولەک",
            parse_mode='HTML')
...
def handle_control(message):
    ...
    if action == "▶️ دەستپێکردن":
        if is_bot_running(user_id, file_name):
            bot.send_message(message.chat.id, "⚠️ کاردەکات.")
        else:
            try:
...
# --- Cleanup: سڕینەوەی هەموو داتا کاتێک بۆت وەستێت ---
def full_cleanup():
    logger.info("🧹 سڕینەوەی داتا دەستپێکرد...")

    # وەستاندنی هەموو subprocess بۆتەکان
    for key in list(bot_scripts.keys()):
        try:
            parts = key.split('_', 1)
            if len(parts) == 2:
    ...
    try:
        if os.path.exists(UPLOAD_BOTS_DIR):
            shutil.rmtree(UPLOAD_BOTS_DIR)
            logger.info("✅ upload_bots سڕایەوە")
    except Exception as e:
        logger.error(f"❌ هەڵە لە سڕینەوەی upload_bots: {e}")

    # سڕینەوەی دیتابەیس
    try:
        if os.path.exists(DATABASE_PATH):
            os.remove(DATABASE_PATH)
            logger.info("✅ دیتابەیس سڕایەوە")
    except Exception as e:
        logger.error(f"❌ هەڵە لە سڕینەوەی دیتابەیس: {e}")

    logger.info("✅ سڕینەوەی داتا تەواو بوو")
